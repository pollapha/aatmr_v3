...
if ($billing_for == 'Customer') {
    ...
    $Summary_Service_Charge = [
        ...
        [5, 'EDC Bailment Empty Package Return Service', $empty_package_row],
        [6, 'Management Fees', '=N' . $management_row],
    ];


    if ($sumNormalBailment == 0) {
        unset($Summary_Service_Charge[0]);
    }
    if ($sumNormalNonBailment == 0) {
        unset($Summary_Service_Charge[1]);
    }
    if ($sumBlowoutBailment == 0) {
        unset($Summary_Service_Charge[2]);
    }
    if ($sumBlowoutNonBailment == 0) {
        unset($Summary_Service_Charge[3]);
    }
    if ($sumEmpty == 0) {
        unset($Summary_Service_Charge[4]);
    }

    // var_dump($Summary_Service_Charge);
    // exit();

    $arr_total = array();

    if ($sumNormalBailment > 0) {
        array_push($arr_total, $sumNormalBailment);
    }
    if ($sumNormalNonBailment > 0) {
        array_push($arr_total, $sumNormalNonBailment);
    }
    if ($sumBlowoutBailment > 0) {
        array_push($arr_total, $sumBlowoutBailment);
    }
    if ($sumBlowoutNonBailment > 0) {
        array_push($arr_total, $sumBlowoutNonBailment);
    }
    if ($sumEmpty > 0) {
        array_push($arr_total, $sumEmpty);
    }

    $i = 0;
    while ($i < count($arr_total) - 1) {
        $Summary_Service_Charge[$i][0] = $i + 1;
        $i++;
    }
} else {
...
if ($billing_for == 'Partner') {
    $filename = 'fileoutput/Summary Service Charge Carrier ' . $carrier . '_' . $project . ' ' . $start_date . '.xlsx';
} else {
