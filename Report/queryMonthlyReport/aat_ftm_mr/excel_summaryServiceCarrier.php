...
foreach ($dataArray as $row) {
    ...
    } elseif (strpos(strtolower($row['description']), 'special') !== false && $row['qty'] > 0) {
        $formattedData = [
            $no_special,
            $row['description_show'],
            $row['truckType'],
...
function addHeaderData($worksheet)
{
    ...
    $contact = 'K.Worapong Chamnanpana';
    $email = 'worapong.c@ttvsuplychain.com';
...
function Blowout_Trip_Delivery($worksheet, $row, $Blowout_Trip_Delivery)
{
    $row += 2;
    $col = ['D', 'E', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O'];
    $data = ['Extra Trip Delivery (Blow Outs)', 'Item', 'Description', 'Truck Type', 'Distance Band', 'Unit', 'Service Rate', 'Qty', 'Amount', 'Drop charge', 'Total'];
    $range = addHeaderTable($worksheet, $data, $row, $col);
    applyBorders($worksheet, $range, 'T0B0R0L0');
    $worksheet->mergeCells('E' . ($row + 1) . ':' . 'G' . ($row + 1));
    $dataStartRow = $row + 2;
    $lastRow = addDetailTable($worksheet, $Blowout_Trip_Delivery, $row, $col);
    for ($i = $dataStartRow; $i < $lastRow; $i++) {
        $worksheet->mergeCells('E' . ($i) . ':G' . ($i));
    }
    $row = $lastRow;
...
$row = Blowout_Trip_Delivery($worksheet, $row, $blowoutArray);
$blowout_row = $row;

function Addnew_Trip_Delivery($worksheet, $row, $Addnew_Trip_Delivery)
{
    $row += 2;
    $col = ['D', 'E', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O'];
    $data = ['Extra Trip Delivery (Additional)', 'Item', 'Description', 'Truck Type', 'Distance Band', 'Unit', 'Service Rate', 'Qty', 'Amount', 'Drop charge', 'Total'];
    $range = addHeaderTable($worksheet, $data, $row, $col);
    applyBorders($worksheet, $range, 'T0B0R0L0');
    $worksheet->mergeCells('E' . ($row + 1) . ':' . 'G' . ($row + 1));
    $dataStartRow = $row + 2;
    $lastRow = addDetailTable($worksheet, $Addnew_Trip_Delivery, $row, $col);
    for ($i = $dataStartRow; $i < $lastRow; $i++) {
        $worksheet->mergeCells('E' . ($i) . ':G' . ($i));
    }
    $row = $lastRow;
...
$row = Addnew_Trip_Delivery($worksheet, $row, $addNewArray);
$addnew_row = $row;
...
function Special_Trip_Delivery($worksheet, $row, $Special_Trip_Delivery)
{
    $row += 2;
    $col = ['D', 'E', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O'];
    $data = ['Special', 'Item', 'Description', 'Truck Type', 'Distance Band', 'Unit', 'Service Rate', 'Qty', 'Amount', 'Drop charge', 'Total'];
    $range = addHeaderTable($worksheet, $data, $row, $col);
    applyBorders($worksheet, $range, 'T0B0R0L0');
    $worksheet->mergeCells('E' . ($row + 1) . ':' . 'G' . ($row + 1));
    $dataStartRow = $row + 2;
    $lastRow = addDetailTable($worksheet, $Special_Trip_Delivery, $row, $col);
    for ($i = $dataStartRow; $i < $lastRow; $i++) {
        $worksheet->mergeCells('E' . ($i) . ':G' . ($i));
    }
    $row = $lastRow;
...
if (count($specialArray) > 0) {
    $row = Special_Trip_Delivery($worksheet, $row, $specialArray);
    $special_row = $row;

    $Summary_Service_Charge = [
        [1, 'Normal Trip Delivery', '=O' . $normal_trip_row],
        [2, 'Extra Trip Delivery (Blow Outsl)', '=O' . $blowout_row],
        [3, 'Extra Trip Delivery (Additional)', '=O' . $addnew_row],
        [4, 'Empty Package Return', '=O' . $empty_package_row],
        [5, 'Special', '=O' . $special_row],
...
$filename = 'fileoutput/Summary Service Charge Carrier ' . $carrier . ' of ' . $project_name . ' ' . $start_date . '.xlsx';
