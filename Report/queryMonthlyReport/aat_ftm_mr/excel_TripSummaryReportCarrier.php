...
foreach ($dataArraySummaryTrip as $row) {
    ...
    } elseif ($row['Work_Type'] == 'Additional') {
        $formattedData = [
            $no2,
            $row['operation_date'],
            $row['Start_Datetime'],
...
function addHeaderSheet2($worksheet)
{
    $cellData = [
        ...
        'D3' => ['value' => 'Master Data Aligned with Blueyonder', 'alignment' => 'left'],
...
function setDefaultStylesSheetConditionalSpecial($worksheet)
{
    $styles = [
        'A' => ['width' => 0.00], 'B' => ['width' => 0.00], 'C' => ['width' => 0.00],
        'D' => ['width' => 10.00], 'E' => ['width' => 15.00], 'F' => ['width' => 15.00], 'G' => ['width' => 15.00],
        'H' => ['width' => 25.00], 'I' => ['width' => 15.00], 'J' => ['width' => 25.00], 'K' => ['width' => 15.00],
        'L' => ['width' => 15.00], 'M' => ['width' => 15.00], 'N' => ['width' => 15.00], 'O' => ['width' => 15.00],
        'P' => ['width' => 15.00], 'Q' => ['width' => 15.00], 'R' => ['width' => 15.00], 'S' => ['width' => 20.00],
    ];
    $worksheet->getParent()->getDefaultStyle()->getFont()->setName('Arial');
...
function addHeaderSheetConditional($worksheet)
{
    $cellData = [
        ...
        'H4' => ['value' => 'Blueyonder Tracking no.', 'alignment' => 'center',],
        'I4' => ['value' => 'Blueyonder Route no.', 'alignment' => 'center',],
...
function trip_summary_report_data($worksheet, $row, $data)
{
    ...
    $borderCode = 'L1R1T1B1';
    $row_border = $row - 1;
    applyBorders($worksheet, 'D6:S' . $row_border, $borderCode);
...
if (count($blowoutArray) > 0) {
    $worksheet_blowout = new Worksheet($spreadsheet, 'Extra Trip (Blow out)');
    $spreadsheet->addSheet($worksheet_blowout, 3);
    $worksheet_blowout->setShowGridlines(false);
    addHeaderSheetConditional($worksheet_blowout);
    setDefaultStylesSheetConditional($worksheet_blowout);
    $row = 4;
    trip_summary_report_conditional($worksheet_blowout, $row, $blowoutArray);
    $worksheet_blowout->setAutoFilter('D4:S4');
}

if (count($addNewArray) > 0) {
    $worksheet_addnew = new Worksheet($spreadsheet, 'Extra Trip (Additional)');
    $spreadsheet->addSheet($worksheet_addnew, 4);
    $worksheet_addnew->setShowGridlines(false);
    addHeaderSheetConditional($worksheet_addnew);
    setDefaultStylesSheetConditional($worksheet_addnew);
    $row = 4;
    trip_summary_report_conditional($worksheet_addnew, $row, $addNewArray);
    $worksheet_addnew->setAutoFilter('D4:S4');
}

if (count($emptyPackageArray) > 0) {
    $worksheet_empty = new Worksheet($spreadsheet, 'Empty Package');
    $spreadsheet->addSheet($worksheet_empty, 5);
...
if (count($specialArray) > 0) {
    $worksheet_special = new Worksheet($spreadsheet, 'Special');
    $spreadsheet->addSheet($worksheet_special, 6);
    $worksheet_special->setShowGridlines(false);
    addHeaderSheetConditional($worksheet_special);
    setDefaultStylesSheetConditionalSpecial($worksheet_special);
    $row = 4;
    trip_summary_report_conditional($worksheet_special, $row, $specialArray);
    $worksheet_special->setAutoFilter('D4:S4');
}
