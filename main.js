var header_toolbar = {
    ...
    elements: [{
        view: "label",
        label: "<a href="+_getRootUrl+"><img class=\'photo\' src=\'images/dscm แนวนอนพื้นขาว.jpg\' height=\'40\' /></a>",
        width: 200
    }, {
        height: 46,
        id: "person_template",
...
webix.event(window, "resize", function()
{
    ...
    $$('renderTitle').resize();
    $$('app:menu').getParentView().define("height", window.innerHeight-55);
    $$('app:menu').getParentView().resize();
    $('.slimScrollDiv').height(window.innerHeight-55+"px");
});
...
function _show_(ar)
{
    for(var i=-1,len=ar.length;++i<len;)$$(ar[i]).show();
}

function _hide_(ar)
{
    for(var i=-1,len=ar.length;++i<len;)$$(ar[i]).hide();
}
...
function ajax(url,params,type,callback,btn,callbackType,obj={})
{
    if(btn) btn.disable();
    ...
    .done(function (data) 
    {
        ...
        else if(json.ch == 2)
        {
            ...
            var params = setView({title:"<b>ข้อความจากระบบ</b>",ok:'ตกลง',text:json.data,callback:function(){}},obj);
            webix.alert(params);
        }
        else if(json.ch == 9)
        {
        ...
        {
            if(isFunction(callbackType)) callbackType(json);
            var params = setView({title:"<b>ข้อความจากระบบ</b>",ok:'ตกลง',text:json.data,callback:function(){window.open("login.php","_self");}},obj);
            webix.alert(params);
        }
    });
};
...
function inputFeed(input,url)
{
    ...
    function nextVertical(tableName,editor)
    {
        var dataTable = tableName;
        var nextRowID = dataTable.getNextId(editor.row);
        if(nextRowID == undefined)
        {
            nextRowID = dataTable.getFirstId();
        }
        dataTable.edit({
            row:nextRowID,
    ...
    function nextHorizontal(tableName,editor)
    {
        var dataTable = tableName;

        var columnConfig = _.filter(dataTable.config.columns, function(o) 
        {
            if(!_.has(o, 'editor')) return false;
            return o.editor.length>0;
        });
        var firstCol=columnConfig[0].id,lastCol=columnConfig[columnConfig.length-1].id,nextCol='';
        if(editor.column == firstCol) 
        {
            if(columnConfig.length>1) nextCol = columnConfig[1].id;
            else nextCol = columnConfig[0].id;                                        
        }
        else if(editor.column == lastCol) nextCol = columnConfig[0].id;
        else
        {
            var indexAr = _.map(columnConfig, function(o) 
            {
                return o.id == editor.column;
            });
            if(indexAr.indexOf(true)+1 <columnConfig.length) nextCol = columnConfig[indexAr.indexOf(true)+1].id;
            else nextCol = columnConfig[0].id;
        }
    ...
    function nextCellVertical(tableName,editor)
    {
        setTimeout(function()
        {
            nextVertical(tableName,editor);
        },100);
    }

    function nextCellHorizontal(tableName,editor)
    {
        setTimeout(function()
        {
            nextHorizontal(tableName,editor);
        },100);
    }

    function cleaveDateFormat(input)
    {
        new Cleave(input, {
            date: true,
            delimiter: '/',
