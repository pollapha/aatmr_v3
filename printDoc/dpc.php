...
/*  $doctype = 'CLC19070200002';
 ...
 class PDF extends PDF_Code128
 {
    ...
    function Header()
    {
      ...
      , 'valign:T;align:L');
      $header->printRow();
      $header->easyCell('DAILY CYCLE COUNT (BY PART)', 'valign:M;align:C;colspan:2;font-size:10;border:TB');
      $header->printRow();
      $header->endTable(0);
      ...
      $header->easyCell(utf8Th("Actual Qty"), 'valign:M;align:C;');
      $header->printRow();
      $header->endTable(0);
 ...
 $sumLine = 0;
 $sumQty = 0;
/*  $sql = "SELECT t1.Document_No,t1.Create_Count_Date,t1.Create_Count_Name,
 t3.Part_Number,t3.Part_Name,t4.Location
 from tbl_nm_inventory_daily_part_body t1
 left join tbl_nm_inventory_daily_part_onhand t2 on t1.Daily_Part_Check_Header=t2.Daily_Part_Check_Header and
 t1.Part_ID=t2.Part_ID
 left join tbl_partmaster t3 on t1.Part_ID=t3.ID
 left join tbl_storagemaster t4 on t2.Location_ID=t4.ID
 where t1.Document_No='$doctype' order by t1.Part_ID;"; */
 
 
 $sql = "SELECT t1.Document_No,t1.Document_Date,t1.Created_Name,
 t1.Status_Header,t1.Check_Type,t3.Part_Number,t3.Part_Name,t4.Location,t1.Station
 from tbl_nm_cyclecount_header t1
 left join tbl_nm_cyclecount_body t2 on t1.ID=t2.Cyclecount_ID
 left join tbl_partmaster t3 on t2.P_LO_ID=t3.ID
 left join view_inventory_for_compare t5 on t2.P_LO_ID=t5.Part_ID
 left join tbl_storagemaster t4 on t5.Location_ID=t4.ID
 where t1.Document_No='$doctype'
 group by t2.P_LO_ID,t5.Location_ID;";
