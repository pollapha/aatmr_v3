...
if(strlen($printerName) == 0 || strlen($doctype) == 0 || strlen($printType) == 0 || strlen($warter) == 0 || $copy == 0) 
 ...
 class PDF extends PDF_Code128
 {
    ...
    function Header()
    {
      ...
      $header->easyCell(utf8Th($v->Customer_Code), 'valign:M;align:L;');
      ...
      $header->easyCell(utf8Th($v->Customer_Name), 'valign:M;align:L;');
      ...
      $header=new easyTable($this->instance, '%{5,30,30,15,10,10}','border:1;font-family:Trirong;font-size:8; font-style:B;');
      ...
      $header->easyCell(utf8Th("GRN"), 'valign:M;align:C;');
 ...
 $sumLine = 0;
 $sumQty = 0;
 $sql = "SELECT t36.Document_NO,dateth(t36.Document_Date)Document_Date,
 t36.Remarks,t36.Status,
 datetimeth(t36.Creation_DateTime)Creation_DateTime,datetimeth(t36.Last_Updated_DateTime)Last_Updated_DateTime,
 t39.Customer_Code,t39.Customer_Name,t39.Customer_Name_TH,t39.Billing_Address,t39.Contact,
 t35.Company_Code,t35.Company_Name,
 t37.Document_Ref,
 sum(t38.Qty)Qty,t38.Status,
 t26.Warehouse_Code,
 ...
 from
 tbl_transaction_outbound t36
 left join tbl_transaction_outbound_line t37 on t36.Transaction_Out_ID=t37.Transaction_Out_ID
 left join tbl_transaction_outbound_detail t38 on t37.Transaction_Out_Line_ID=t38.Transaction_Out_Line_ID
 left join tbl_transaction_inbound_line t33 on t38.Transaction_In_Line_ID=t33.Transaction_In_Line_ID
 left join tbl_transaction_inbound t32 on t33.Transaction_In_ID=t32.Transaction_In_ID
 left join tbl_customer_items t28 on t37.Product_ID=t28.Cus_Item_ID
 left join tbl_items_master t27 on t28.Item_ID=t27.Item_ID
 left join tbl_warehouse_master t26 on t38.Warehouse_ID=t26.Warehouse_ID
 left join tbl_customer_master t39 on t36.Customer_ID=t39.Customer_ID
 left join tbl_companys t35 on t36.Company_ID=t35.Company_ID
 where t36.Document_NO='$doctype' and t38.Status='ACTIVE'
 group by t37.Transaction_Out_ID,t38.Transaction_Out_Line_ID";
 ...
 $colWidths = '%{5,30,30,15,10,10}';
 $header=new easyTable($pdf, $colWidths,'border:1;font-family:Trirong;font-size:7; font-style:B;');
 $numRow = 0;
 $Cus_Code = '';
 $totalQty = 0;
 $subTotalQty = 0;
 ...
 for($i=0,$len=count($data);$i<$len;$i++)
 {
  ...
  $header->easyCell(utf8Th($rowObj->Document_Ref), 'valign:M;align:L;');
  $header->easyCell(utf8Th($rowObj->Qty), 'valign:M;align:R;');
  $header->easyCell(utf8Th($rowObj->UOM), 'valign:M;align:L;');
  $header->printRow();
  
  $subTotalQty += $rowObj->Qty;
  
 }
 $totalQty += $subTotalQty;
 subTotal($header,$subTotalQty);
 Total($header,$totalQty);
 $header->endTable(0);
 ...
 $header->easyCell(utf8Th("Transfer By"), 'valign:M;align:C;');
 $header->printRow();
...
function subTotal($header,$subTotalQty)
{
  $header->easyCell(utf8Th('Sub Total'), 'valign:M;align:R;colspan:4');
  $header->easyCell(utf8Th($subTotalQty), 'valign:M;align:R;');
  $header->easyCell(utf8Th(''), 'valign:M;align:C;');
  $header->printRow();
}

function Total($header,$totalQty)
{
  $header->easyCell(utf8Th('Total'), 'valign:M;align:R;colspan:4');
