...
$tbody = array();
 ...
 where t1.PO_NO='$doctype' and t1.Transaction_Type='PURCHSE' and t2.Status='APPLY'
...
while ($i <  $bodyLen) 
{
    ...
    /* FOT115X55X25=>FOT1150550250
IMC190=>IMC190RSC
FOT1660X2240X710=>2240X1660X710
FOT1660X2240X730=>2240X1660X730
IMC190=>IMC190HC
IMC190HC=>IMC190HC(SNP5) */
    /* $checkPO = array('01/01/20');
    if(!in_array($dataHeader->{'PO_Date'},$checkPO))
    {
      $detail->easyCell(utf8Th($data[$i]->{'Supplier_Part_No'}), 'align:C;');
    }
    else
    {
      $partDummy = '';
      if($data[$i]->{'Supplier_Part_No'} == 'FOT115X55X25')
      {
        $partDummy = 'FOT1150X550X250';
      }
      else if($data[$i]->{'Supplier_Part_No'} == 'IMC190')
      {
        if($data[$i]->{'Customer_Project'} == 'ABT-TSPT')
        {
          $partDummy = 'IMC190RSC';
        }
        else 
        {
          $partDummy = $data[$i]->{'Supplier_Part_No'};
        }
        
      }
      else if($data[$i]->{'Supplier_Part_No'} == 'FOT1660X2240X710')
      {
        $partDummy = '2240X1660X710';
      }
      else if($data[$i]->{'Supplier_Part_No'} == 'FOT1660X2240X730')
      {
        $partDummy = '2240X1660X730';
      }
      else if($data[$i]->{'Supplier_Part_No'} == 'IMC190')
      {
        if($data[$i]->{'Customer_Project'} == 'ABT-TSPT')
      ...
      else if($data[$i]->{'Supplier_Part_No'} == 'IMC190HC')
      {
        $partDummy = 'IMC190HC(SNP5)';
      }
      else
      {
        $partDummy = $data[$i]->{'Supplier_Part_No'};
      }
      $detail->easyCell(utf8Th($partDummy), 'align:C;');     
    }  */
