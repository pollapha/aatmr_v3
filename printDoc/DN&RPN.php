...
$tbody = array();
 ...
 $sumLine = 0;
 $sumQty = 0;
 $sql = "SELECT t40.RPN_NO,dateth(t40.RPN_Date)RPN_Date,t40.DN_NO,dateth(t40.DN_Date)DN_Date,t40.Remarks,t40.Status,
 datetimeth(t40.Creation_DateTime)Creation_DateTime,datetimeth(t40.Last_Updated_DateTime)Last_Updated_DateTime,
 t1.BPO_NO,dateth(t1.BPO_Date)BPO_Date,
 t41.Qty_Delivery,
 ...
 from tbl_vendor_order t40
 left join tbl_vendor_order_line t41 on t40.Vendor_Order_ID=t41.Vendor_Order_ID
 left join tbl_vendor_master t25 on t40.Vendor_ID=t25.Vendor_ID
 left join tbl_warehouse_master t26 on t40.Warehouse_ID=t26.Warehouse_ID
 left join tbl_bpo_detail t3 on t41.BPO_Detail_ID=t3.BPO_Detail_ID
 left join tbl_bpo_line t2 on t3.BPO_Line_ID=t2.BPO_Line_ID
 left join tbl_bpo t1 on t2.BPO_ID=t1.BPO_ID
 ...
 left join tbl_user t30 on t40.Created_By_ID=t30.user_id
 left join tbl_user t31 on t40.Updated_By_ID=t31.user_id
 where RPN_NO='$doctype' and t41.Status='ACTIVE';";
...
class PDF extends PDF_Code128
{
  ...
  function Header()
  {
    ...
    $header->printRow();
    $header->easyCell(' <s font-size:16;font-style:B;>DELIVERY ORDER(REPLEN)</s>', 'colspan:2;align:C;border:T');
    $header->easyCell('', '', '');
    $header->printRow();  
    $header->endTable(0);
    ...
    $header->easyCell('DELIVERY ORDER NO. :', 'align:L;border:T;font-style:B;');
    $header->easyCell($v->{'RPN_NO'}, 'align:L;border:TR;');
    $header->printRow();
    ...
    $header->easyCell($v->{'RPN_Date'}, 'align:L;border:R;');
    $header->printRow();
    ...
    $header->easyCell($v->{'DN_Date'}, 'align:L;border:R;');
    $header->printRow();

    $header->easyCell('CONTACT :', 'align:L;border:L;font-style:B;');
    $header->easyCell($v->{'Contact'}, 'align:L;');
    $header->easyCell('BPO NO :', 'align:L;border:L;font-style:B;');
    $header->easyCell($v->BPO_NO, 'align:L;border:R;');
    $header->printRow();
...
while ($i <  $bodyLen) 
{
    ...
    if($i == 0)
    {
      $itemCode = $data[$i]->{'Cus_Code'};
    }

    if($itemCode != $data[$i]->{'Cus_Code'})
    {
      $end = 1;
      $itemCode = $data[$i]->{'Cus_Code'};
    }
    ...
    $detail->easyCell(utf8Th($nn), 'align:C');
      ...
      $detail->easyCell(utf8Th($data[$i]->{'Cus_Code'}.' '.$data[$i]->{'Item_Name'}), 'align:C');
      ...
      $detail->easyCell(number_format(utf8Th($data[$i]->{'Qty_Delivery'}),2), 'align:R');
      // $total += $data[$i]->{'Amount'};
      $total += 1;
      $detail->printRow();
    $i++;
