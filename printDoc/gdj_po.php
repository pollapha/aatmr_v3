...
$tbody = array();
 ...
 from(SELECT t1.Customer_Project,t1.Document_NO,date_format(t1.PO_Date,'%d/%m/%Y')Document_Date,
  t1.DN_NO,date_format(t1.DN_Date,'%d/%m/%Y')DN_Date,t1.PO_NO,date_format(t1.PO_Date,'%d/%m/%y')PO_Date,
  t1.Remarks,t1.Transaction_Type,t1.Status,t1.PO_Txt,
  round(sum(t2.Qty)) Qty,t2.Unit_Price,round(sum(t2.Qty)*t2.Unit_Price,2) Amount,
 ...
 t14.Zipcode
  ...
  where t1.PO_NO='$doctype' and t1.Transaction_Type='PURCHSE' 
  group by t1.PO_NO,t1.Customer_Project,t2.Item_ID,t2.Unit_Price) s1
  left join
 (SELECT t1.Document_NO,t4.Customer_Part_No,t2.Item_ID
 from tbl_transaction_header t1
 left join tbl_transaction_body t2 on t1.ID=t2.Transaction_ID
 left join tbl_transaction_detail t3 on t2.ID=t3.Transaction_Body_ID
 left join tbl_parts_master t4 on t3.Part_ID=t4.ID
 where t1.PO_NO='$doctype' and t1.Transaction_Type='PURCHSE'
 group by t1.PO_NO,t1.Customer_Project,t2.Item_ID)s2 
 on s1.Document_NO=s2.Document_NO and s1.Item_ID=s2.Item_ID;";
...
class PDF extends PDF_Code128
{
  ...
  function Header()
  {
    ...
    $header->easyCell('PO Number :', 'align:L;');
    $header->easyCell($v->{'PO_NO'}, 'align:L;font-style:B;font-size:8;');
    $header->printRow();
    
    
    $header->easyCell('Customer Name :', 'align:L;');
    $header->easyCell($v->{'Trader_Name'}, 'align:L;');
    $header->easyCell('PO Date :', 'align:L;');
    $header->easyCell($v->{'PO_Date'}, 'align:L;font-style:B;font-size:8;');
    $header->printRow();
    ...
    $header->printRow();
    $header->easyCell('Remarks : '.strtoupper($v->{'PO_Txt'}), 'colspan:4;valign:T;paddingY:0.4;');
    $header->printRow();
    $header->endTable(0);
