...
$tbody = array();
 ...
 t14.Zipcode
  ...
  where t1.Invoice_NO='$doctype' and t1.Transaction_Type='SOLD' 
...
while ($i <  $bodyLen) {
    ...
    /* $checkInvoice = array('01/01/20');
    if(!in_array($dataHeader->{'Invoice_Date'},$checkInvoice))
    {
    ...
    {
      $partDummy = '';
      if($data[$i]->{'Customer_Part_No'} == 'FOT115X55X25')
      {
        $partDummy = 'FOT1150X550X250';
      }
      else
      {
        $partDummy = $data[$i]->{'Customer_Part_No'};
      }
      $headdetail->easyCell(utf8Th($partDummy), 'align:L;border:LR;');
    } */
