...
$mysqli = mysqli_init();
if (!$mysqli) {
  echo json_encode(array('ch' => 2, 'data' => 'mysqli_init failed'));
  exit();
}
try {
  $mysqli->real_connect('127.0.0.1', 'root', '1234', 'aatmr_v2_test', 3306);
  //$mysqli->real_connect('dbl01.ttv-supplychain.com', 'pollapha', 'Vk!D4$dnh8#K%z', 'aatmr_v2_test', 3306);
  $mysqli->set_charset("utf8");
  $mysqli->options(MYSQLI_OPT_INT_AND_FLOAT_NATIVE, TRUE);
  $mysqli->query("SET SESSION sql_mode = 'NO_ENGINE_SUBSTITUTION,STRICT_TRANS_TABLES,NO_ZERO_DATE,NO_ZERO_IN_DATE,ERROR_FOR_DIVISION_BY_ZERO'");
} catch (mysqli_sql_exception $e) {
  echo json_encode(array('ch' => 2, 'data' => 'ไม่สามารถติดต่อฐานข้อมูลได้'));
  exit();
}
/* connectionLimit : 10,
host            : 'ttvdbs.titan-vns.com',
user            : 'apichat',
password        : '!Q2w3e4r',
database        : 'aatmr_v2_test',
port            : 3306 */
