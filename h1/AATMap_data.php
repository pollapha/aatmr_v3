...
include('../start.php');
session_start();
if(!isset($_SESSION['xxxID']) || !isset($_SESSION['xxxRole']) || !isset($_SESSION['xxxID']) || !isset($_SESSION['xxxFName'])  || !isset($_SESSION['xxxRole']->{'AATMap'}) )
{
	echo "{ch:10,data:'เวลาการเชื่อมต่อหมด<br>คุณจำเป็นต้อง login ใหม่'}";
	exit();
}
else if($_SESSION['xxxRole']->{'AATMap'}[0] == 0)
{
...
if($type<=10)//data
{
	...
	{
		$re1 = getDataGeoSupplier($mysqli,__LINE__);
	...
	else if($type == 3)
	{
		...
		$sql = "SELECT t1.code from tbl_supplier t1 where (t1.code like 'GRBNA-%' or t1.code='GRBNA') and t1.code like '%$value%'";
		if($value == '') echo '[]';
		else toArrayStringOne($mysqli->query($sql),1);
	}
	else if($type == 5)
	{
...
{
	if($_SESSION['xxxRole']->{'AATMap'}[1] == 0) closeDBT($mysqli,9,'คุณไม่ได้รับอุญาติให้ทำกิจกรรมนี้');
	if($type == 11)
	{
...
{
	if($_SESSION['xxxRole']->{'AATMap'}[2] == 0) closeDBT($mysqli,9,'คุณไม่ได้รับอุญาติให้ทำกิจกรรมนี้');
	if($type == 21)
	{
	...
	else if($type == 22)
	{
		...
		$sql = "
		UPDATE tbl_supplier2 SET geo=ST_GeomFromText('$data'),updateBy='$fName',updateDatetime=now() where code='$supplier_code' limit 1;
		";
...
{
	if($_SESSION['xxxRole']->{'AATMap'}[3] == 0) closeDBT($mysqli,9,'คุณไม่ได้รับอุญาติให้ทำกิจกรรมนี้');
	if($type == 31)
	{
...
function getDataGeoSupplier($mysqli)
{
	...
	from tbl_supplier2 t1 where t1.code like 'GRBNA-%' or t1.code='GRBNA';";
