<?php
    ...
    s1.statusTypeSort,timeFormat(s1.planTimeIn)planTimeIn,timeFormat(s1.planTimeOut)planTimeOut
    ...
    {
        $pdfBuff = trim(file_get_contents('http://localhost:'.$port.'/mazdamr/print/pus.php?doctype='.$row['ID'].'&copy=1&printType=F&printerName=1401&warter=NO'));
        $merge->add($pdfBuff);
        unlink($pdfBuff);
    }
