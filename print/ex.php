...
{ 
    ...
    {  
        ...
        // unlink($fd.$pdfBuff);
        $projectManual = array('Manual  SCM','E-Smart','PxP MR');
        ...
        if($projectName[0] == "FTM MR")
        {
            $pdfBuff = file_get_contents('http://'.$host.':'.$port.'/aatmr_v3/print/truckControl_FTM.php?doctype='.$doctype.
            '&copy=1&printType=F&printerName=1401&warter=NO');
            $merge->add($fd.$pdfBuff);
            unlink($fd.$pdfBuff);

            foreach ($data as $key => $value) {
                $pdfBuff = file_get_contents('http://'.$host.':'.$port.'/aatmr_v3/print/pickup_FTM.php?doctype='.$value.'&copy=1&printType=F&printerName=1401&warter=NO');
                $merge->add($fd.$pdfBuff);
                unlink($fd.$pdfBuff);
        ...
        {
          $pdfBuff = file_get_contents('http://'.$host.':'.$port.'/aatmr_v3/print/truckControl.php?doctype='.$doctype.
          '&copy=1&printType=F&printerName=1401&warter=NO');
          $merge->add($fd.$pdfBuff);
          unlink($fd.$pdfBuff);
          ...
          foreach ($data as $key => $value) {
              if(in_array($projectName[0], $projectManual)) 
              {
                  
              } 
              else 
              {
                $pdfBuff = file_get_contents('http://'.$host.':'.$port.'/aatmr_v3/print/pickupByidAll.php?doctype='.$value.'&copy=1&printType=F&printerName=1401&warter=NO');
                $merge->add($fd.$pdfBuff);
                unlink($fd.$pdfBuff);
              }
          }

          $pdfBuff = file_get_contents('http://'.$host.':'.$port.'/aatmr_v3/print/packageControl.php?doctype='.$doctype.'&copy=1&printType=F&printerName=1401&warter=NO');
          $merge->add($fd.$pdfBuff);
          unlink($fd.$pdfBuff);
        }
        // exit();
