...
h.orderID,h.ld_supplierCode,substring_index(h.ld_supplierName,' ',2)ld_supplierName,h.ld_dueDate,h.ld_dueTime,h.ud_dueDate,h.ud_dueTime,o.TM_DELIVERY,o.DT_DELIVERY,
o.partNo,o.CD_PLANT_DOCK_LOC,o.dockGroup,QT_SHP_DEL qty,o.QT_PKG snp,round(o.QT_SHP_DEL/o.QT_PKG)box,p.boxType,
round(QT_SHP_DEL*p.weight) wt,o.NO_PGM,o.CD_PLANT,round(p.box_w*p.box_l*p.box_h/1000000,2)*round(o.QT_SHP_DEL/o.QT_PKG) cbm
...
where h.ID=$doctype and ediStatus='ACTIVE' order by o.DT_DELIVERY,o.TM_DELIVERY,o.CD_PLANT_DOCK_LOC,p.partNo,p.boxType;
...
function col2($pdf,$row,$col=2)
{
    ...
    $pdf->Cell(40,5,'PENSKE TRACKING NO :',0,0,'R',false);
    $pdf->SetFont('Arial', '', 8);
    ...
    $pdf->SetFont('Arial', '', 8);
    $pdf->Cell(40,5,substr($row->bol,3,9),'B',0,'L',false);
...
function createTableBody($pdf,$row)
{
    ...
    $pdf->Cell($col[1],5,date('Y-m-d H:i',strtotime($row->ld_dueDate.' '.$row->ld_dueTime)),1,0,'C',false);
