...
// 
      ...
      $header=new easyTable($this->instance, '%{17,14,17,27,25}','border:0;font-family:Trirong;font-size:7; font-style:B;');
      $header->easyCell('PUS DATE', 'border:TL;valign:M;align:C;');
      $header->easyCell('PROMISED DATE', 'border:TL;valign:M;align:C;');
      $header->easyCell('PUS NUMBER', 'border:TL;valign:M;align:L;');
      ...
      $header=new easyTable($this->instance, '%{5,23,36,10,8,8,10}','border:1;font-family:Trirong;font-size:7; font-style:B;bgcolor:#e0e0e0;');
      $header->easyCell('NO.', 'valign:M;align:C');
      $header->easyCell(utf8Th("PO Number / NEED BY DATE"), 'valign:M;align:C');
      ...
      $header->printRow();
      $header->endTable(0);
      $this->instance->Code128(94.5,38.5,$this->headerData->pusChild,55,5);
...
/*  MakeFont('Trirong-Bold.ttf','cp874');
 ...
 $sumLine = 0;
 $sumQty = 0;
 $sql = "SELECT t1.pusChild,t1.pusDate,t3.po_number,t3.qty,t3.line,t3.mst_need_date,t3.supplier_promised_date,
 t4.supplier_name_en,t4.supplier_code,t4.supplier_address_th,
 t4.supplier_contact,t4.supplier_phone,t5.part_number,t5.part_name,
 t6.buyer_phone,t6.buyer_name_en,t6.buyer_phone
 from tbl_transaction t1 left join tbl_transaction_detail t2 on t1.ID=t2.transaction_ID
 ...
 where pusChild='$doctype' order by t3.po_number*1;";
 ...
 $header=new easyTable($pdf, '%{5,23,36,10,8,8,10}','border:1;font-family:Trirong;font-size:7; font-style:B;');
 $po = '';
 for($i=0,$len=count($data);$i<$len;$i++)
 {
    ...
    $header->easyCell(utf8Th($data[$i]->part_number."\n".$data[$i]->part_name), 'valign:T;align:C;');
    ...
    {
      $po = $data[$i]->po_number;
      $pdf->Code128(24,$pdf->GetY()-7.8,$data[$i]->po_number,35,6);
    }
    $pdf->Code128(70,$pdf->GetY()-7.8,$data[$i]->part_number,55,6);
