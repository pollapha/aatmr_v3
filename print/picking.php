...
if($result = $mysqli->query("SELECT `order`,cby_pick cBy,cdate_pick date,ddate,TIME_FORMAT(t1.dTime,'%H:%i') dtime,concat(t2.supplierPartNo,' | ',t2.partNo)partNo,t1.doc,t1.inv,t1.dock,(t1.qty)qty,floor((t1.qty)/t2.snpIN)box,t2.snpIN,if((t1.qty)<t2.snpIN,0,MOD((t1.qty),t2.snpIN)) partial,t2.pick  from tbl_order t1 left join tbl_partmaster t2 on t1.partID=t2.id
where doc='$doctype' order by t1.dDate,t1.dTime,t1.`order`;")) 
{ 
    $len = $result->num_rows;
    if($len > 0)
    {  
